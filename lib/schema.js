    ...
    parse: function(schema, namespace) {
        ...
        if (_.isNull(schema) || _.isUndefined(schema)) {
            ...
            if (schema.type === 'record') {
                var newRecord = new RecordSchema(schema.name, schema.namespace, _.map(schema.fields, function(field) {
                    return Schema(field, namespace);
                }));
                return newRecord;
            } else if (schema.type === 'enum') {
                return new EnumSchema(schema.symbols);
            } else if (schema.type === 'array') {
                return new ArraySchema(schema.items, namespace);
            } else if (schema.type === 'map') {
                if (_.has(schema, 'values')) {
                    return new MapSchema(_parseSchema(schema.values, schema, namespace));
                } else {
                    throw new AvroInvalidSchemaError('map must specify "values" schema, got ' + JSON.stringify(schema));
                }
            } else if (schema.type === 'fixed') {
               return new FixedSchema(schema.size);
            } else if (_.has(schema, 'type') && _.contains(PRIMITIVE_TYPES, schema.type)) {
