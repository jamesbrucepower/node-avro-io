            ...
            if (schema.type === 'record') {
                if (!_.has(schema, 'fields')) {
                    ...
                    return new RecordSchema(schema.name, schema.namespace, 
                                            _.reduce(schema.fields, function(hash, field) {
                                                hash[field.name] = new FieldSchema(field.name, self.parse(field, namespace)); 
                                                return hash;
                                            }, {}));
                }
...
function RecordSchema(name, namespace, fields) {
    ...
    if (!_.isObject(fields) || _.isArray(fields)) {
        throw new AvroInvalidSchemaError('Fields must be a hash');
    }
