...
var _parseNamedType = function(schema, namespace) {
    if (_.contains(Schema.PRIMITIVE_TYPES, schema)) {
        return schema;
    } else {
...
var Schema = new function(schema, namespace) {

    this.PRIMITIVE_TYPES = ['null', 'boolean', 'int', 'long', 'float', 'double', 'bytes', 'string'],
    this.COMPLEX_TYPES = ['record', 'enum', 'array', 'map', 'union', 'fixed'];
    
    if ((this instanceof arguments.callee) === false)
        return new arguments.callee(schema, namespace);
        
    if (!_.isUndefined(schema))
        this.parse(schema, namespace);
}
...
Schema.prototype = {
    ...
    parse: function(schema, namespace) {
        if (_.isNull(schema) || _.isUndefined(schema)) {
            throw new AvroInvalidSchemaError('schema is null, in parentSchema: ' + JSON.stringify(parentSchema));
        } else if (_.isString(schema)) {
            this.type = _parseNamedType(schema, namespace);
            ...
            if (schema.type === 'record') {
                ...
                if (_.has(schema, 'symbols')) {
                    ...
                    throw new AvroInvalidSchemaError('enum must specify symbols, got ' + JSON.stringify(schema));
                }
            } else if (schema.type === 'array') {
                if (_.has(schema, 'items')) {
                    return new AvroArray(_parseSchema(schema.items, schema, namespace));
                } else {
                    throw new AvroInvalidSchemaError('array must specify "items" schema, got ' + JSON.stringify(schema));
                }
            } else if (schema.type === 'map') {
                if (_.has(schema, 'values')) {
                    return new Map(_parseSchema(schema.values, schema, namespace));
                } else {
                    throw new AvroInvalidSchemaError('map must specify "values" schema, got ' + JSON.stringify(schema));
                }
            } else if (_.has(schema, 'type') && _.contains(AvroSpec.PrimitiveTypes, schema.type)) {
                return _parseNamedType(schema.type, namespace);
            } else {
                throw new AvroInvalidSchemaError('not yet implemented: ' + schema.type);
            }
...
var NamedSchema = _.extend(Schema)
