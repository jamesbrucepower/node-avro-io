...
BinaryDecoder.prototype = {
    ...
    readByte: function() {
        return this._input.read(1).readUInt8(0);
    },
...
DatumReader.prototype = {
  ...
  //          console.log("processing data for %s", field.name);
  //          console.log(readersSchema);
    ...
    readBlocks: function(decoder, lambda){
        ...
        while(count) { 
            if (count < 0) {
                ...
                console.log("Detected negative block count of %d, block size %d, \n%s", count, blockSize, arguments.callee.caller);
                count = -count;
            }
            while(count--) lambda();
    ...
    skipBlocks: function(decoder, lambda) {
        this.readBlocks(decoder, lambda);
    }
}
...
DatumWriter.prototype = {
    ...
    writeUnion: function(writersSchema, datum, encoder) {
        ...
        for (var i = 0; i < writersSchema.schemas.length; i++) {
            //console.log("%d/%d: %j ==? %j", i, writersSchema.schemas.length, writersSchema.schemas[i], _.keys(datum)[0]);
