...
DatumReader.prototype = {
    ...
    readArray: function(writersSchema, readersSchema, decoder) {
        var self = this;
        var anArray = [];
        if (this.readBlocks(decoder, function() {
    ...
    skipArray: function(writersSchema, decoder) {
        var self = this;
        if (this.skipBlocks(decoder, function() {
    ...
    readMap: function(writersSchema, readersSchema, decoder) {
        ...
        var block = this.readBlocks(decoder, function() {
            ...
            if (key === null) 
                ...
                value = self.readData(writersSchema.values, readersSchema.values, decoder); 
            if (map !== null)
                map[key] = value;
        });
        if (block === null)
    ...
    readUnion: function(writersSchema, readersSchema, decoder) {
        ...
        if (schemaIndex < 0 || schemaIndex >= writersSchema.schemas.length) {
            throw new AvroIOError("Union %j is out of bounds for %d, %d, %d", writersSchema, schemaIndex, decoder._input.offset, decoder._input.length);   
        }
        ...
        union[selectedWritersSchema.name || selectedWritersSchema.type] = data;
    ...
    readRecord: function(writersSchema, readersSchema, decoder) {
        ...
        _.each(writersSchema.fields, function(field) {
            ...
            console.error("fieldName: %s, fieldType: %s", field.name, field.type);
            if (readersField) {
                try {
                    var data = self.readData(field.type, readersField.type, decoder); 
                    if (writersSchema.name == 'org.apache.avro.block') {
                        console.error('Field: %s(%d), Type: %s, %s', field.name, Buffer.isBuffer(data) ? data.length : -1, field.type, util.inspect(data));
                    }
                    record[field.name] = data;
                }
                catch (e) {
                    console.error(e);
                    console.error('going back %d', oldOffset - decoder._input.offset);
                    decoder._input.skip(oldOffset - decoder._input.offset);
                    throw(e);
                }                    
            } else {
