...
function getSchemaType(schema) {
    console.log("schema(%s) is %j", typeof(schema), schema);
    if (_.isArray(schema)) {
        return { type: "union" };
    } else if (schema.type) {
        if (_.isArray(schema.type)) 
            return { type: "union" };
        else if (_.isObject(schema.type))
            return schema.type;
        else
            return schema;
    } else 
...
DatumReader.prototype = {
    ...
    readData: function(writersSchema, readersSchema, decoder) {
        ...
        if (!(writersSchema instanceof Schema))
            writersSchema = Schema(writersSchema);
    ...
    skipData: function(writersSchema, decoder) {
        ...
        var schema = getSchemaType(writersSchema);
        switch(schema.type) {
            ...
            case "fixed":   return decoder.skipFixed(schema.size); break;
            ...
            default:
                throw new AvroIOError("Unknown type: %s", schema);
        }
    },
    ...
    readArray: function(writersSchema, readersSchema, decoder) {
        ...
        while(blockCount) {
            for (var i = 0; i < blockCount; i++) {
                anArray.push(this.readData(writersSchema.items, readersSchema ? readersSchema.items : null, decoder));
            }
            blockCount = Math.abs(decoder.readLong());
        }
        return anArray;
    },
    ...
    readMap: function(writersSchema, readersSchema, decoder) {
        var map = {};
        var blockCount = Math.abs(decoder.readLong());
        //console.log("blockcount is %d", blockCount);
        while(blockCount) {
            for (var i = 0; i < blockCount; i++) {
                var key = decoder.readString();
                //console.log("key=%s, %j(%s)", key, writersSchema, typeof(writersSchema));
                map[key] = this.readData(writersSchema.type.values, readersSchema ? readersSchema.type.values : null, decoder);
            }
    ...
    skipMap: function(writersSchema, decoder) {
        ...
        this.skipBlocks(decoder, function() {
            decoder.skipString();
            self.skipData(writersSchema.type.values, decoder);
        })
    },

    readUnion: function(writersSchema, readersSchema, decoder) {
        var schemaIndex = decoder.readLong(); 
        if (!_.isArray(writersSchema))
            writersSchema = writersSchema.type;

        if (schemaIndex < 0 || schemaIndex >= writersSchema.length) {
            console.log(decoder._input.offset);
            console.log(decoder._input.toBuffer().slice(70, 90));
            throw new AvroIOError("Union of size %d is out of bounds at %d", writersSchema.length, schemaIndex);   
        }
    ...
    skipUnion: function(writersSchema, decoder) {
        ...
        this.skipData(writersSchema[index], decoder)
    },
    
    readRecord: function(writersSchema, readersSchema, decoder) {
        var self = this;
        var record = {};
        console.log("record %s has %d fields\n%j", writersSchema.name, _.size(writersSchema.fields), writersSchema);
        _.each(writersSchema.fields, function(field) {
            // TODO: fixme
            //console.log("record field %s is %j, offset %d",field.name, field.type, decoder._input.offset);
            if (readersSchema) {
                //console.log("SKIPPING %s", field.type);
                self.skipData(field.type, decoder);
            } else
                record[field.name] = self.readData(field, null, decoder); 
        });
        //console.log(_.keys(record));
        return record;
    },
    ...
    skipRecord: function(writersSchema, decoder) {
        var self = this;
        _.each(writersSchema.fields, function(field) {
            self.skipData(field, decoder);
        })
    },
    
    skipBlocks: function(decoder, datumSkipFunction) {
        var count = decoder.readLong();
        while(count) { 
            while(count--) datumSkipFunction();
...
DatumWriter.prototype = {
    ...
    writeUnion: function(writersSchema, datum, encoder) {
        var schemaIndex = -1; 
        if (_.isArray(writersSchema.type))
            writersSchema = writersSchema.type;
            
        for (var i = 0; i < writersSchema.length; i++) {
            console.log(writersSchema[i]);
            if (writersSchema[i] === _.keys(datum)[0] || 
                (writersSchema[i].type === "record" && writersSchema[i].name === _.keys(datum)[0]))
                schemaIndex = i;
        }
        ...
        if (schemaIndex < 0) {
            ...
            this.writeData(writersSchema[schemaIndex], datum[writersSchema[schemaIndex]], encoder);
        }
    },
    ...
    writeRecord: function(writersSchema, datum, encoder) {
        ...
        _.each(writersSchema.fields, function(field) {
            if (field.name == "exception")
                debugger;
            self.writeData(_.isObject(field.type) ? field.type : field, datum[field.name], encoder); 
        });
    }
}
