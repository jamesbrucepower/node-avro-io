...
var df = require(libpath + "datafile");
...
BinaryDecoder.prototype = {
    ...
    readFixed: function(len) {
        if (this.idx + len > this.buf.length)
            throw new Error("oob for readFixed in current Buffer");
        else {
            this.idx += len;
    ...
    readBytes: function() {
        ...
        if (this.idx + len > this.buf.length) 
            throw new Error("oob for readBytes in current Buffer");
        else
    ...
    seek: function(offset) {
        var newIdx = this.idx + offset;
        if (newIdx < 0 || newIdx > this.buf.length)
            throw new Error("Out of range on seek");
        else
            this.idx = newIdx;
    },
}
