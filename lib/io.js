...
BinaryDecoder.prototype = {
    ...
    readBytes: function() {
        ...
        if (len == 1884) debugger;
        if (len && len > 0) {
            var bytes = this.readFixed(len);
            if (bytes) return bytes;
            else {
                //console.error('len is %d, going back %d - %d, %d', len, this._input.offset, oldOffset, this._input.length);
    ...
    readRecord: function(writersSchema, readersSchema, decoder) {
        ...
        _.each(writersSchema.fields, function(field) {
            ...
            if (readersField) {
                ...
                if (field.type != 'boolean' && data === null) {
                    decoder._input.skip(oldOffset - decoder._input.offset);
                    return null;
                } else
