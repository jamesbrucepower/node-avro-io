...
DatumReader.prototype = {
    ...
    readUnion: function(writersSchema, readersSchema, decoder) {
        ...
        var union = {};
        union[selectedWritersSchema.name || selectedWritersSchema.type] = this.readData(selectedWritersSchema, readersSchema, decoder);
    ...
    readRecord: function(writersSchema, readersSchema, decoder) {
        ...
        _.each(writersSchema.fields, function(field) {
            var readersField = readersSchema.fields[field.name];
            if (readersField) {
    ...
    skipBlocks: function(decoder, datumSkip) {
        var count = decoder.readLong();
        while(count) { 
            while(count--) datumSkip();
