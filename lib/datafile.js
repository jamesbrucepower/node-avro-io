...
_.extend(Reader.prototype, {
    ...
    _readHeader: function() {
        ...
        if (header instanceof Error) {
            ...
            throw new AvroErrors.FileError("Not an avro file, header was %j", header);
        }
    ...
    _readBlock: function(final) {
        ...
        if (!final && block instanceof AvroErrors.BlockDelayReadError) {
            // If its not the final block then delay until end of runloop
            ...
            // If the sync marker doesn't match, maybe there is no sync mark, try skipping back
            ...
            this.decompressData(block.objects, this.codec, function(err, data) {
                if (err) 
					self.emit('error', new Error(util.format("codec %s, blockNum %d: %j", self.codec, block.blockNum, err)));
                else {
					if (data) {
    ...
    write: function(newBuffer) {
		...
		// TODO: don't think we should have to wait for remainingBytes > SYNC_SIZE
        while (this._fileBlock.remainingBytes > SYNC_SIZE && 
			   this._fileBlock.offset !== previousOffset) {
            this.blockNum++;
            this._readBlock();
