...
console.error("%s, %j", libpath, AvroErrors);
...
_.extend(Block.prototype, {
    ...
    read: function(size) {
        ...
        if (size > this.remainingBytes) {
            return new AvroErrors.BlockDelayReadError('tried to read(%d) past the amount written to the block(%d) at %d', 
                                      size, this.remainingBytes, this._readOffset);
        } else if (this._readOffset + size > this._buffer.length) {
            throw new AvroErrors.BlockError('tried to read(%d) outside of the buffer(%d) at %d', 
...
_.extend(Reader.prototype, {
    ...
    _readHeader: function() {
        ...
        if (header instanceof AvroErrors.BlockReadHeadError || !header.magic || header.magic.toString() !== magic()) {
    ...
    _readBlock: function(final) {
        ...
        if (block instanceof AvroErrors.BlockReadHeadError) {
