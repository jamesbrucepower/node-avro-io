...
var AvroFile = function() {
    ...
    this.open = function(path, schema, options) {    
        ...
        switch (_options.flags) {
            ...
            _operation.decoder = IO.BinaryDecoder(_operation);
...
Block.prototype = {
    ...
    skip: function(size) {
        if (size > 0 && size < (this.length - this._offset))
            this._offset += size;
    },
    ...
    read: function(size) {
        if (size > 1) {
            this._offset += size;
            return this._buffer.slice(this._offset - size, this._offset);
        } else
    ...
    writeBlock: function(callback) {
        ...
        if (this._blockCount > 0) {
            self.compressData(self._datumBlock.toBuffer(), self.codec, function(err, buffer) {
                ...
                self.encoder.setOutput(self._fileBlock);
                ...
                self._offset = 0;
                self._blockCount = 0;    
                self.encoder.setOutput(self._datumBlock);
                callback();
            });
        } else 
        callback();
