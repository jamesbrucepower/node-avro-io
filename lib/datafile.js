...
DataFile.prototype = {
    ...
    read: function(callback) {
        ...
        this.stream.on("data", function(buffer) {
            console.log("idx %d, got buffer of length %d", this.fileOffset, buffer.length);
            self.decoder.setBuffer(buffer);
            if (this.fileOffset == 0)
                this.fileOffset += self.readHeader();
            console.log("read header idx now %d", this.fileOffset);
            while (this.fileOffset <= buffer.length) {
