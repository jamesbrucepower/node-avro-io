...
_.extend(Reader.prototype,  {
    ...
    read: function(length) {
        //console.log("length is %d, bytes buffered is %d", length, this.bytesAhead());
        if (length > this.bytesAhead()) {
            return null; //throw new Error("oob for readByte in current Buffer");
        } else if (length == 1)
            return this._buffer[this._moveOffset(1)];
        else { 
            this._moveOffset(length);   
            var newBuffer = new Buffer(length);
            //console.log("%d -> %d", this._offset - length, this._offset);
...
DataFile.prototype = {
    ...
    readBlock: function(callback) {
        ...
        var offset = this.reader._offset;
        ...
        if (block.objects) {
            ...
            if (block.sync.toString() !== this.syncMarker.toString()) {
                this.reader.skip(-this.SYNC_SIZE);
            }
    ...
    read: function(callback) {
        ...
        this.reader.on("data", function() {
            ...
            while (self.reader.bytesAhead() > self.SYNC_SIZE && self.readBlock(callback)) {}
        });
    },
    
    close: function() {
        var self = this;
        if (this.encoder && this.blockCount > 0)
