    ...
    this.open = function(path, schema, options) {    
        ...
        switch (_options.flags) {
            ...
            case "w":
                ...
                _operation.writersSchema = Avro.Schema(schema);
...
Block.prototype = {
    ...
    _bufferSize: function(size) {
        if (!this._buffer.length) {
            ...
            return this._buffer.length + size;
        }
    },
    ...
    _resizeIfRequired: function(size) {
        if (this._canReUseBuffer(size)) {
            this._buffer.copy(this._buffer, this.offset - newBuffer.length, this._offset);
    ...
    read: function(size) {
        if (size > this.remainingBytes() || this.offset + size > this._buffer.length) {
            return new Buffer(0);
        } else if (size >= 0) {
            this.offset += size;
            //console.log("%d / %d", this.offset, this._buffer.length);
            return this._buffer.slice(this.offset - size, this.offset);
        } else {
            throw new AvroBlockError("Tried to read %d bytes", size);
        }
    },
...
_.extend(Reader.prototype, {
    ...
    decompressData: function(data, codec, callback) {
        switch(codec) {
            ...
            case "snappy":  snappy.decompress(data.slice(0,-4), snappy.parsers.raw, callback);  break;
            default:        callback(new AvroFileError("Unsupported codec %s", codec));         break;
        }
    },
