...
Block.prototype = {
    ...
    skip: function(size) {
        if (size > 0 && size <= this.remainingBytes())
            ...
            throw new AvroFileError('tried to skip(%d) outsite of the block(%d) at %d', size, this.remainingBytes(), this.offset);
    },
...
_.extend(Reader.prototype, {
    ...
    read: function(schema, callback) {
        ...
        else if (_.isUndefined(this.decoder)) {
            throw new AvroFileError("Decoder is not defined");
        }
