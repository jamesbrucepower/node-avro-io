...
_.extend(Block.prototype, {
    ...
    _resizeIfRequired: function(size) {
        if (this._canReUseBuffer(size)) {
            //console.error("REUSE for %d, remaining is %d (bufsize = %d)", size, this.remainingBytes, this._buffer.length);
            ...
            //console.error("EXTENDING by %d, to %d", size, this._buffer.length + size);
    ...
    skip: function(size) {
        if (size == 0)
            return;
        else if ((this._readOffset + size) > 0 && size <= this.remainingBytes)
...
_.extend(Reader.prototype, {
    ...
    _readHeader: function() {
        try {
            ...
            if (!header.magic || header.magic.toString() !== magic()) {
            ...
            console.error('SYNC: ' + util.inspect(header.sync));
    ...
    _readBlock: function(final) {
        ...
        console.error('READING block at %d', oldOffset);
        var block = this.datumReader.readData(blockSchema(), null, this.decoder);
        console.error('BLK %s', util.inspect(block));
        ...
        if (block && block.objects) {
            ...
            if (block.sync && block.sync.toString() !== this.syncMarker.toString()) {
                console.error("FILEBLOCK size %d, offset %d", this._fileBlock.length, this._fileBlock.offset);
                console.error("going back %d " + util.inspect(block) + " (%d)", SYNC_SIZE, block.objects.length);
                this._fileBlock.skip(-SYNC_SIZE);
            ...
            this.decompressData(block.objects, this.codec, function(err, data) {
                if (err) self.emit('error', err);
                    ...
                    console.error("%d decompressed %d blocks %d -> %d bytes, comp:%d, uncomp:%d", block.blockNum, block.objectCount, block.objects.length, data.length, Math.round(data.length / block.objectCount), Math.round(block.objects.length / block.objectCount));
        ...
        } else if (this._fileBlock.remainingBytes > 0 && final) {
            console.error("delaying... and trying again %d", this._fileBlock.remainingBytes);
            process.nextTick(function() {
                self._readBlock(final);
            });
        } 
    ...
    write: function(newBuffer) {
        ...
        console.error('%d STREAM %d, FILEBLOCK %d/%d, DATUMBLOCK %d/%d, newBuffer %d', this.blockNum, this._streamOffset, this._fileBlock.offset, this._fileBlock.length, this._datumBlock.offset, this._datumBlock.length, newBuffer.length);
