...
var DataFile = function() {
    ...
    _.extend(Writer.prototype, {
        ...
        writeBlock: function(data, callback) {
			...
			var encoder = IO.BinaryEncoder(compressedBlock);
            if (_blockCount > 0) {
                self.compressData(data, _options.codec, function(err, buffer) {
                    self.writer.writeData(blockSchema(), blockData(buffer), encoder);
                    fs.writeSync(self.fd, compressedBlock.toBuffer());
                    self._offset = 0;
                    _blockCount = 0;    
                    callback(err);
                });
        ...
        write: function(data, callback) {
            ...
            if (this._offset > this.SYNC_INTERVAL) {
                this.writeBlock(this._rawBlock.toBuffer(), function(err) {
                    callback(err);
                });
            } else
                callback();
