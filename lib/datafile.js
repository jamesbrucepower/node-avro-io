...
DataFile.prototype = {
    ...
    readHeader: function(){
        ...
        var header = this.reader.readData(schema, schema, this.decoder);
    ...
    read: function(callback) {
        ...
        this.stream.on("data", function(buffer) {
            ...
            if (shouldReadHeader) {
                self.fileOffset += self.readHeader();
                shouldReadHeader = false;
            }
            console.log("read header offset now %d", self.fileOffset);
            while (self.fileOffset <= buffer.length) {
                console.log("reading at offset %d", self.fileOffset);
                self.readBlock(function(err, data) {
                    if (err)
                        callback(err);
                    else {
                        self.fileOffset += self.decoder.idx;  
                        callback(null, data);
                        self.stream.resume();
                    }              
                });   
